...
export type ThemeVariables = Record<
    'primary' | 'primaryDeep' | 'primaryMild' | 'primarySubtle' | 'muted',
    string
>
...
function useThemeSchema() {
    ...
    const applyTheme = (theme: string): void => {
        if (presetThemeSchemaConfig[theme][mode as 'dark' | 'light']) {
            const themeObject = mapTheme(
                presetThemeSchemaConfig[theme][mode as 'dark' | 'light'],
            )
